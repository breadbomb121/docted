...
pub fn get_doc(item: String, overide_lang: Option<String>) -> Result<()> {
    ...
    match lang.as_str() {
        "java" | "react" | "javascript" => tokio::runtime::Runtime::new()?.block_on(search(item, lang))?,
        _ => return Err(anyhow!("No language selected"))
    };
    Ok(())
}
async fn search(item: String, lang: String) -> Result<()>{
